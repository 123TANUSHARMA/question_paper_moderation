...
print("=== 🧠 Question Paper Generation & Moderation Pipeline ===\n")
sys.stdout.flush()

base_path = os.path.dirname(os.path.abspath(__file__))

scripts = [
    os.path.join(base_path, "Pytesseract/without_flask.py"),
    os.path.join(base_path, "questions_generation/question_gen.py"),
    os.path.join(base_path, "Store_and_embed/ollama_store.py")
]

# === 1️⃣ Run without_flask.py automatically with arguments ===
print(f"🚀 Running: {scripts[0]}")
sys.stdout.flush()

# These arguments simulate user input:
# "_" = skip text input, next arg = path to file
ocr_file_path = "/Users/tanusharma/Downloads/coe-project/Pytesseract/pdf.pdf"

try:
    result = subprocess.run(
        ["python", scripts[0], "_", ocr_file_path],
        check=True,
        capture_output=True,
        text=True
    )
    ...
    print(f"❌ Error in {scripts[0]}:\n", e.stderr)
    sys.exit(1)

# === 2️⃣ Run question generation ===
print(f"\n🚀 Running: {scripts[1]}")
sys.stdout.flush()
try:
    result = subprocess.run(
    ...
    print(f"❌ Error in {scripts[1]}:\n", e.stderr)
    sys.exit(1)

# === 3️⃣ Run FAISS + storage script ===
print(f"\n🚀 Running: {scripts[2]}")
sys.stdout.flush()
try:
    result = subprocess.run(
